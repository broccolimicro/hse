...
vector<cycle> get_cycles(graph &g, bool report_progress) {
	...
	while (frames.size() > 0)
	{
		...
		cout << endl;
		for (int i = 0; i < (int)curr.sim.ready.size(); i++)
			cout << "(" << i << ")\t" << curr.sim.loaded[curr.sim.ready[i].first].guard_action.to_string(variables) << " -> " << g.transitions[curr.sim.loaded[curr.sim.ready[i].first].index].local_action.cubes[curr.sim.ready[i].second].to_action(variables) << endl;
		*/
...
graph to_petri_net(graph &g, bool report_progress) {
	...
	*/
	...
	for (int i = 0; i < (int)cycles.size(); i++) {
		cout << "Cycle " << i << endl;
		for (int j = 0; j < (int)cycles[i].firings.size(); j++) {
			cout << "\t(" << cycles[i].firings[j].action.index << " " << cycles[i].firings[j].action.term << ")\t" << cycles[i].firings[j].guard.to_string(g) << " -> " << g.transitions[cycles[i].firings[j].action.index].local_action.cubes[cycles[i].firings[j].action.term].to_action(g) << "\t{";
	...
	for (int i = 0; i < (int)result.transitions.size(); i++) {
		...
		if (loop) {
			...
			cout << "Found Loop " << result.transitions[i].local_action.to_action(g) << endl;
		}
	}
	...
	for (int i = 0; i < (int)con.size(); i++) {
		...
		for (int j = 0; j < (int)con[i].first.size(); j++) {
			if (j != 0)
				cout << " ";
			cout << result.transitions[con[i].first[j]].local_action.to_action(g);
		}
		cout << "} => " << result.transitions[con[i].second].local_action.to_action(g) << endl;
