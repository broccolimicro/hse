...
void encoder::check(bool senseless, bool report_progress) {
	...
	for (int j = 0; j < (int)base->transitions.size(); j++) {
		...
		for (int term = 0; term < (int)local_action.cubes.size(); term++) {
			...
			//cout << "checking " << t0 << " " << implicant.to_string(*variables) << " " << to_string(relevant) << " -> " << export_composition(action, *variables).to_string() << endl;
			...
			for (int sense = senseless ? -1 : 0; senseless ? sense == -1 : sense < 2; sense++) {
				...
				//cout << "\tterm_implicant " << term_implicant.to_string(*variables) << endl;
				...
				for (auto i = relevant.begin(); i != relevant.end(); i++) {
					...
					//cout << "\t against " << *i << " " << encoding.to_string(*variables) << endl;
					...
					//cout << "\t final encoding " << encoding.to_string(*variables) << endl;
...
int encoder::score_insertion(int sense, petri::region pos, const petri::path_set &dontcare) {
	...
	for (auto i = relevant.begin(); i != relevant.end(); i++) {
		...
		//cout << "\t\tlooking at " << *i << " for " << implicant.to_string(*variables) << " vs " << base->effective_implicant(vector<petri::iterator>(1, *i)).to_string(*variables) << endl;
