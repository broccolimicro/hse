...
TEST(Simulator, SimpleLoop) {
    ...
    // Simple linear structure: p0->t0->p1->t1->p0
    auto p = g.create(place(), 2);
    auto t = g.create(transition(), 2);
   
		int x = g.create(net("x", 0));

		g.transitions[t[0].index].guard = boolean::cube(1);
		g.transitions[t[0].index].local_action = boolean::cube(x, 1);
		g.transitions[t[0].index].remote_action = boolean::cube(x, 1);
		g.transitions[t[1].index].guard = boolean::cube(1);
		g.transitions[t[1].index].local_action = boolean::cube(x, 0);
		g.transitions[t[1].index].remote_action = boolean::cube(x, 0);
 
    g.connect({p[0], t[0], p[1], t[1], p[0]});
    
    // Set initial state with token on p0
    g.reset.push_back(state(vector<token>({token(p[0].index)}), boolean::cover(x, 0)));
    ...
    // Verify initial state
		...
		EXPECT_EQ(sim.tokens[0].index, p[0].index);
		EXPECT_EQ(sim.encoding, boolean::cover(x, 0)); 
    
    // Get enabled transitions
    // Verify that the correct transition is enabled
    EXPECT_EQ(sim.enabled(), 1);
    EXPECT_EQ(sim.ready.size(), 1);
    EXPECT_EQ(sim.loaded.size(), 1);
		EXPECT_EQ(sim.ready[0].first, 0);
		EXPECT_EQ(sim.loaded[sim.ready[0].first].index, t[0].index);
    ...
    // Verify token moved from p0 to p1
		EXPECT_EQ(sim.tokens.size(), 1u);
		EXPECT_EQ(sim.tokens[0].index, p[1].index);
    ...
    EXPECT_EQ(sim.loaded.size(), 1);
		EXPECT_EQ(sim.ready[0].first, 0);
		EXPECT_EQ(sim.loaded[sim.ready[0].first].index, t[1].index);
